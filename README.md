...
Dự án NCKH với cấu trúc Frontend và Backend, được containerized bằng Docker.
...
### Frontend (Node.js)
- Port: 3000
- Hot-reload enabled thông qua volume mounting
...
1. Nếu gặp lỗi port đã được sử dụng:
   - Kiểm tra và dừng các process đang sử dụng port 3000 hoặc 5000
   - Hoặc thay đổi port trong docker-compose.yml

2. Nếu container không build được:
   - Xóa node_modules và build lại: `docker-compose up -d --build`
   - Kiểm tra logs: `docker-compose logs`

3. Nếu có thay đổi trong dependencies:
   - Cập nhật requirements.txt hoặc package.json
   - Build lại container: `docker-compose up -d --build`
