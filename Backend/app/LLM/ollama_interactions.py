...
def call_ollama_stream_logic(ollama_api_url, payload, stage_key, analysis_results_ref, conversation_history_ref):
    ...
    Modifies analysis_results_ref and conversation_history_ref directly.
    """
    full_response_content = ""
    try:
        response = requests.post(ollama_api_url, json=payload, stream=True, timeout=400) # Tăng timeout nếu model xử lý chậm
                    ...
                    if json_chunk.get("done"):
                        ...
                        if stage_key == "stage3_tonghop":
                            if payload.get("messages"):
                                ...
                                conversation_history_ref.append({"role": "assistant", "content": full_response_content})
                            else:
                                conversation_history_ref[:] = [{"role": "assistant", "content": full_response_content}]
        ...
        error_message = f"Lỗi: Timeout khi gọi Ollama API cho {stage_key}."
        print(error_message)
